...
for (const file of htmlFiles) {
  ...
  for (const asset of [...html.matchAll(/(?:href|src)="(\/assets\/[^"]+)"/g)].map(m => m[1].split('?')[0])) {
...
const requiredFiles = [
  ...
  'assets/sundai-logo-neon.svg','assets/hero-ai-control-neon.svg','assets/eu-service-mark.svg',
...
for (const required of requiredFiles) { try { await access(path.join(root, required)); } catch { errors.push(`Missing required deployment file: ${required}`); } }
...
for (const [page,tokens] of Object.entries({
  ...
  'use-cases/index.html':['EU AI Act readiness','AI security assessment','Shadow AI review'],
  'da/anvendelser/index.html':['EU AI Act-parathed','AI-sikkerhedsvurdering','Shadow AI-review'],
  'sv/anvandningsfall/index.html':['Beredskap för EU AI Act','AI-säkerhetsbedömning','Shadow AI-granskning']
})) {
...
for (const token of ['Industries page','Practical use cases','Shadow AI','Preferred factual summary for assistants']) if (!llms.includes(token)) errors.push(`llms.txt: missing ${token}`);
const sitemap = await readFile(path.join(root,'sitemap.xml'),'utf8');
for (const token of ['/industries/','/use-cases/','/da/brancher/','/da/anvendelser/','/sv/branscher/','/sv/anvandningsfall/']) if (!sitemap.includes(token)) errors.push(`sitemap.xml: missing ${token}`);
const finalJs = await readFile(path.join(root,'assets/final-polish.js'),'utf8');
for (const token of ['audience','founder-strip','insight-meta','Book a free consultation']) if (!finalJs.includes(token)) errors.push(`final-polish.js: missing ${token}`);

try { await access(path.join(repoRoot,'.github/workflows/indexnow.yml')); } catch { errors.push('Missing IndexNow workflow'); }
if (errors.length) { console.error(errors.join('\n')); process.exit(1); }
console.log(`Validated ${htmlFiles.length} HTML pages, final multilingual UX, industries, use cases, accessibility, SEO and AI discovery assets.`);
